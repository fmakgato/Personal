...
namespace CarpIn
{
    public class CreatePdf
    {
        ...
        public void printPdf()
        {
            ...
            Billings billE = new Billings();
            //DataRow drPayee = DataRow.Equals(object w, object q);
            try
            {
                ...
                PdfWriter writer = PdfWriter.GetInstance(doc, new FileStream("Invoice", FileMode.Create));
                doc.Open();
                //Inserting Image
                ...
                writeText(cb, "Amount", left_margin + 500, top_margin, f_cb, 10);
                //inserting a grid
                PdfPTable table = new PdfPTable(billE.dgvInvoice.Columns.Count);
                for (int i = 0; i < billE.dgvInvoice.Columns.Count; i++)
                {
                    table.AddCell(new Phrase(billE.dgvInvoice.Columns[i].HeaderText));
                }
                table.HeaderRows = 1;

                for (int a = 0; a < billE.dgvInvoice.Rows.Count; a++)
                {
                    for (int b = 0; b < billE.dgvInvoice.Columns.Count; b++)
                    {
                        if (billE.dgvInvoice[b, a].Value != null)
                        {
                            table.AddCell(new Phrase(billE.dgvInvoice[b, a].Value.ToString()));
                ...
                PdfWriter writer = PdfWriter.GetInstance(document, new FileStream(billE.txtInvoiceNo.Text.ToString(), FileMode.Create));
                ...
                document.AddAuthor("Mikael Blomquist");
                    ...
                    MessageBox.Show("Invoiced saved to the invoice folder. Good job!");
